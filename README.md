...
🎥 **[Watch the demo recording](docs/demo.webm)** — forms list → select Form D →
open the source picker → map `email` from Form A (transitive ancestor) → map
`dynamic_object` from global data → clear a mapping.

| Forms list | Source picker | Mapped field |
|---|---|---|
| ![Forms list](docs/01-form-list.png) | ![Source picker](docs/02-source-picker.png) | ![Mapped field with clear](docs/03-mapped.png) |

The picker screenshot shows the three source sections for Form D: **Direct
Dependencies** (Forms B, C, E), **Transitive Dependencies** (Form A), and
**Global Data**.
...
The *why* behind these choices — normalized options, raw-vs-internal types, the
transitive definition, reducer state — lives in [docs/DESIGN.md](docs/DESIGN.md).
